...
const EditGroup = ({ user }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(err.error || 'Une erreur est survenue lors de la modification du groupe');
    } finally {
      setLoading(false);
    }
  };

  if (fetchLoading) return <p>Chargement des données du groupe...</p>;
    ...
    <div className="group-form-container">
      <form className="group-form" onSubmit={handleSubmit}>
        ...
        <div className="form-footer">
          <Link to={`/groups/${id}`} className="form-cancel-btn">Annuler</Link>
          <button type="submit" className="form-submit-btn" disabled={loading}>
            {loading ? 'Modification en cours...' : 'Mettre à jour le groupe'}
          </button>
        </div>
      </form>
    </div>
